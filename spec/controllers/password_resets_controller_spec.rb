  ...
  context 'POST #create' do
    context 'for valid email' do
      ...
      it 'sends reset password email' do
        expect { post :create, params: { email: user.email } }.to change { ActionMailer::Base.deliveries.count }.by(1)
      end
    ...
    context 'for invalid email' do
      it 'does not send reset password email' do
        expect { post :create, params: { email: 'jake.peralta@nypd.com' } }.not_to change { ActionMailer::Base.deliveries.count }
      end
      it 'redirects to root_url with proper alert' do
